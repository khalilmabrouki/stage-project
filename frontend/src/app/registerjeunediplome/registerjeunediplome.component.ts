...
@Component({
  selector: 'app-registerjeunediplome',
  templateUrl: './registerjeunediplome.component.html',
  styleUrls: ['./registerjeunediplome.component.css']
})

export class RegisterjeunediplomeComponent {
  ...
  get specialite() { return this.registerForm.get('specialite'); }
  get anneesExperience() { return this.registerForm.get('anneesExperience'); }
  ...
  register() {
    ...
    if (this.registerForm.invalid) {
      ...
      // Parcourir tous les champs pour collecter les erreurs
    ...
    console.log('📝 Jeune diplômé à inscrire:', jeuneDiplome);

    // Appel au service
    this.service.registerJeuneDiplome(jeuneDiplome).subscribe({
      next: (res) => {
        console.log('✅ Inscription réussie:', res);
        Swal.fire({
          icon: 'success',
          title: 'Inscription réussie !',
          text: 'Votre compte jeune diplômé a été créé avec succès. Vous pouvez maintenant vous connecter.',
          timer: 3000,
          showConfirmButton: true
        }).then(() => {
